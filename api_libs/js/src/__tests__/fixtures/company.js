/* @flow */

export default {
  data: {
    email             : 'contact@chartreuse-medical.com',
    formatted_address : '2 Oxford Street, London, United-Kingdom',
    name              : 'Chartreuse high-tech medical device',
    types             : [
      'Supplier',
      'Service Provider',
    ],
    description : 'Diagnostic Apparatus, Medical',
    founded     : '1847',
    industries  : [
      'Hospital, Health Care',
      'Medical Device',
    ],
    linkedin_url : 'https : //www.linkedin.com/company/chartreuse-medical',
    phone        : '+49 89 63600',
    size         : '+ 10 000 employees',
    website_url  : 'http  : //www.chartreuse-medical.com/index.html',
  },
  result: {
    email            : 'contact@chartreuse-medical.com',
    formattedAddress : '2 Oxford Street, London, United-Kingdom',
    name             : 'Chartreuse high-tech medical device',
    types            : [
      'Supplier',
      'Service Provider',
    ],
    description : 'Diagnostic Apparatus, Medical',
    founded     : '1847',
    industries  : [
      'Hospital, Health Care',
      'Medical Device',
    ],
    linkedinUrl : 'https : //www.linkedin.com/company/chartreuse-medical',
    phone       : '+49 89 63600',
    size        : '+ 10 000 employees',
    websiteUrl  : 'http  : //www.chartreuse-medical.com/index.html',
  },
};
